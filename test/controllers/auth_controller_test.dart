...
Future<void> _settle(ProviderContainer c) =>
    _settleUntil(c, (s) => s is! AuthInitializing);

/// Pumps the event loop until [pred] returns true or 100 iterations pass.
Future<void> _settleUntil(
    ProviderContainer c, bool Function(AuthState) pred) async {
...
void main() {
  ...
  group('Initialisation', () {
    ...
    test('→ AuthAuthenticated when token valid + /auth/me succeeds', () async {
      ...
      await _settleUntil(
          c, (s) => s is AuthAuthenticated && s.profile.id == 'user-1');
  ...
  group('verifyOtp', () {
    ...
    test('re-throws ServerError(401) on expired OTP', () async {
      ...
      await expectLater(
        () => c.read(authControllerProvider.notifier).verifyOtp(_kPhone, _kCode),
        throwsA(
            isA<ServerError>().having((e) => e.statusCode, 'statusCode', 401)),
      );
    });
    ...
    test('re-throws ServerError(429) on too many attempts', () async {
      ...
      await expectLater(
        () => c.read(authControllerProvider.notifier).verifyOtp(_kPhone, _kCode),
        throwsA(
            isA<ServerError>().having((e) => e.statusCode, 'statusCode', 429)),
      );
    });
